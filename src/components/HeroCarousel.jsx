 ...
 * Fix: iframe is only rendered for the ACTIVE slide (not hidden with opacity).
 * This stops audio/video from playing in the background on inactive slides.
 */
...
const HeroCarousel = () => {
  ...
  // Track which slides have STARTED rendering (for crossfade)
  ...
  /* ── VIDEO BACKGROUND ── */
  const VideoBackground = ({ videoId, active }) => {
    if (!videoId) return null;
    // KEY on videoId+active: destroys iframe when slide goes inactive
    return (
      <div key={active ? `active-${videoId}` : `inactive-${videoId}`}
        style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        {active && (
          <iframe
            src={bunnyUrl(videoId)}
  ...
  return (
    <section style={{
      ...
      {slides.map((s, i) => {
        ...
        const isRendered = rendered[i];

        // Don't render slides that haven't been visited yet
        ...
        return (
          <div key={s.id || i} style={{
            ...
            {s.type === 'video' && s.backgroundVideoId ? (
              // Only mount iframe when this slide is active — unmount when inactive
              <VideoBackground videoId={s.backgroundVideoId} active={isActive} />
            ) : s.backgroundImage ? (
              <img
