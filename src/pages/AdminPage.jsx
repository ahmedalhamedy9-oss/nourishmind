...
const AdminPage = () => {
  ...
  useEffect(()=>{
    ...
    getDocs(collection(db,'tools')).then(async snap=>{
      if(snap.empty){
        const seed={name:'PsychDecide',subtitle:'Clinical Decision Support · Nutritional Psychiatry',description:'أداة دعم القرار السريري المتكاملة للأطباء وأخصائيي الصحة النفسية وأخصائيي التغذية',icon:'🧠',image:'',path:'/tools/clinical',badge:'Physician',badgeColor:'#4a9b8e',available:true};
        const ref=await addDoc(collection(db,'tools'),seed);
        setTools([{id:ref.id,...seed}]);
      } else {
    ...
    <div className="min-h-screen flex flex-col bg-[#0a1412] font-sans">
      ...
      <div className="flex pt-14 min-h-screen">
        ...
        <main className="ml-52 flex-1 p-8 overflow-y-auto">
          ...
          {/* USERS */}
          {activeTab==='users'&&(<div>{sectionTitle('Users')}<div className="relative mb-6 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500"/><Input value={userSearch} onChange={e=>setUserSearch(e.target.value)} placeholder="Search by email or name..." className="pl-9"/></div><div className="flex flex-col gap-3">{filteredUsers.map(user=>(<details key={user.id} className="bg-[#0d1a17] border border-white/10 rounded-xl overflow-hidden"><summary className="flex items-center gap-4 p-4 cursor-pointer list-none hover:bg-white/5 transition-colors">{user.avatar?<img src={user.avatar} alt={user.name} className="w-9 h-9 rounded-full object-cover shrink-0 border border-white/10"/>:<div className="w-9 h-9 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-sm shrink-0">{user.name?.[0]?.toUpperCase()||user.email?.[0]?.toUpperCase()||'?'}</div>}<div className="flex-1 min-w-0"><p className="text-white font-semibold text-sm truncate">{user.name||'No name'}</p><div className="flex gap-3 text-xs text-gray-500 flex-wrap"><span>{user.email}</span>{user.phone&&<span>{user.phone}</span>}</div></div><div className="flex items-center gap-2 shrink-0"><span className={`text-xs font-bold px-2 py-0.5 rounded-full ${user.status==='suspended'?'bg-red-500/20 text-red-400':user.status==='deleted'?'bg-gray-500/20 text-gray-400':user.status==='paused'?'bg-yellow-500/20 text-yellow-400':user.role==='admin'?'bg-primary/20 text-primary':'bg-white/5 text-gray-400'}`}>{user.status==='suspended'?'Suspended':user.status==='deleted'?'Deleted':user.status==='paused'?'Paused':user.role||'student'}</span><span className="text-xs text-gray-500">{user.enrolledCourses?.length||0} courses</span><ChevronDown className="w-4 h-4 text-gray-500"/></div></summary><div className="border-t border-white/10 p-5 flex flex-col gap-5"><div><p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Edit Name</p><div className="flex gap-2"><Input defaultValue={user.name||''} id={`name-${user.id}`} placeholder="Full name" className="max-w-xs"/><Btn onClick={async()=>{const val=document.getElementById('name-'+user.id)?.value;if(!val)return;await updateDoc(doc(db,'users',user.id),{name:val});}}><Check className="w-4 h-4"/>Save</Btn></div></div><div><p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Enrolled Courses</p><div className="flex flex-wrap gap-2 mb-3">{user.enrolledCourses?.map(cid=>{const c=courses.find(x=>x.id===cid);return(<div key={cid} className="flex items-center gap-1.5 bg-primary/10 border border-primary/20 text-primary text-xs px-2 py-1 rounded-lg">{c?.title||cid}<button onClick={async()=>{const updated=(user.enrolledCourses||[]).filter(id=>id!==cid);await updateDoc(doc(db,'users',user.id),{enrolledCourses:updated});}} className="text-red-400 hover:text-red-300 ml-1 font-bold">x</button></div>);})}  {!user.enrolledCourses?.length&&<p className="text-gray-600 text-xs">No courses enrolled</p>}</div><div className="flex gap-2"><select id={`enroll-${user.id}`} className="bg-[#0a1412] border border-white/10 text-white rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-primary"><option value="">Add to course...</option>{courses.filter(c=>!user.enrolledCourses?.includes(c.id)).map(c=><option key={c.id} value={c.id}>{c.title}</option>)}</select><Btn variant="ghost" className="text-xs py-1.5" onClick={async()=>{const sel=document.getElementById('enroll-'+user.id);if(!sel?.value)return;const updated=[...(user.enrolledCourses||[]),sel.value];await updateDoc(doc(db,'users',user.id),{enrolledCourses:updated});sel.value='';}}><Plus className="w-3 h-3"/>Enroll</Btn></div></div><div><p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Clinical Tools Access</p><div className="flex flex-wrap gap-2 mb-2">{(user.clinicalTools||[]).map(tid=>{const t=tools.find(x=>x.id===tid);return(<div key={tid} className="flex items-center gap-1.5 bg-teal-500/10 border border-teal-500/20 text-teal-400 text-xs px-2 py-1 rounded-lg">{t?.name||tid}<button onClick={async()=>{const updated=(user.clinicalTools||[]).filter(id=>id!==tid);await updateDoc(doc(db,'users',user.id),{clinicalTools:updated});}} className="text-red-400 hover:text-red-300 ml-1 font-bold">x</button></div>);})}{!(user.clinicalTools||[]).length&&<p className="text-gray-600 text-xs">No tool access</p>}</div><div className="flex gap-2"><select id={`tool-${user.id}`} className="bg-[#0a1412] border border-white/10 text-white rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-primary"><option value="">Add tool access...</option>{tools.filter(t=>!(user.clinicalTools||[]).includes(t.id)).map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select><Btn variant="ghost" className="text-xs py-1.5" onClick={async()=>{const sel=document.getElementById('tool-'+user.id);if(!sel?.value)return;const updated=[...(user.clinicalTools||[]),sel.value];await updateDoc(doc(db,'users',user.id),{clinicalTools:updated});sel.value='';}}><Plus className="w-3 h-3"/>Add</Btn></div></div><div className="flex flex-col gap-3 pt-2 border-t border-white/10"><div className="flex items-center gap-2 flex-wrap"><span className="text-xs text-gray-500">Account status:</span><span className={`text-xs font-bold px-2 py-0.5 rounded-full ${user.status==='suspended'?'bg-red-500/20 text-red-400':user.status==='paused'?'bg-yellow-500/20 text-yellow-400':user.status==='deleted'?'bg-gray-500/20 text-gray-400':'bg-green-500/20 text-green-400'}`}>{user.status==='paused'&&user.pausedUntil?`Paused until ${new Date(user.pausedUntil).toLocaleDateString()}`:(user.status||'active')}</span></div><div className="flex gap-2 flex-wrap items-center">{user.status!=='active'&&<Btn variant="ghost" className="text-xs" onClick={async()=>{await updateDoc(doc(db,'users',user.id),{status:'active',pausedUntil:null});}}><Check className="w-3 h-3"/>Activate</Btn>}<div className="flex items-center gap-1"><input type="date" id={`pause-${user.id}`} className="bg-[#0a1412] border border-white/10 text-white rounded-lg px-2 py-1 text-xs focus:outline-none focus:border-primary"/><Btn variant="ghost" className="text-xs" onClick={async()=>{const v=document.getElementById('pause-'+user.id)?.value;if(!v){alert('Pick a date first');return;}await updateDoc(doc(db,'users',user.id),{status:'paused',pausedUntil:new Date(v+'T23:59:59').toISOString()});}}>⏸ Pause until</Btn></div>{user.status!=='suspended'&&<Btn variant="danger" className="text-xs" onClick={async()=>{if(!confirm('Suspend '+(user.name||user.email)+' permanently?'))return;await updateDoc(doc(db,'users',user.id),{status:'suspended',pausedUntil:null});}}>⛔ Suspend</Btn>}<Btn variant="danger" className="text-xs" onClick={async()=>{if(!confirm('DELETE '+(user.name||user.email)+'? Their access is removed immediately. (Soft delete — the login record itself remains.)'))return;await updateDoc(doc(db,'users',user.id),{status:'deleted',deletedAt:new Date().toISOString(),pausedUntil:null});}}><Trash2 className="w-3 h-3"/>Delete</Btn></div></div></div></details>))}{!filteredUsers.length&&<p className="text-gray-600 text-sm">No users found.</p>}</div></div>)}

          {activeTab==='tools'&&(<div>
            {sectionTitle('Clinical Tools', <button onClick={()=>setToolModal({id:'',name:'PsychDecide',subtitle:'Clinical Decision Support · Nutritional Psychiatry',description:'أداة دعم القرار السريري المتكاملة للأطباء وأخصائيي الصحة النفسية وأخصائيي التغذية',icon:'🧠',image:'',path:'/tools/clinical',badge:'Physician',badgeColor:'#4a9b8e',available:true})} className="flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-xl text-white" style={{background:'linear-gradient(135deg,#4a9b8e,#5bb8c4)'}}><Plus className="w-3.5 h-3.5"/> Add Tool</button>)}
            <div className="flex flex-col gap-3">
              {tools.map(tool=>(
                <div key={tool.id} className="flex items-center gap-4 bg-[#0d1a17] border border-white/10 rounded-xl p-4 hover:border-primary/30 transition-colors">
                  <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 flex items-center justify-center" style={{background:'rgba(74,155,142,0.08)',border:'1px solid rgba(74,155,142,0.15)'}}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-teal-400 mb-0.5">{tool.subtitle}</p>
              ...
              <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={()=>setToolModal(null)}>
                <div className="bg-[#0d1a17] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
                  ...
                  <div className="p-6 flex flex-col gap-4">
                    ...
                    <Field label="Description"><Textarea value={toolModal.description||''} onChange={e=>setToolModal(t=>({...t,description:e.target.value}))} rows={2}/></Field>
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="Icon (emoji)"><Input value={toolModal.icon||'🧠'} onChange={e=>setToolModal(t=>({...t,icon:e.target.value}))}/></Field>
                      <Field label="Route Path"><Input value={toolModal.path||''} onChange={e=>setToolModal(t=>({...t,path:e.target.value}))}/></Field>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="Badge Text"><Input value={toolModal.badge||''} onChange={e=>setToolModal(t=>({...t,badge:e.target.value}))}/></Field>
                      <Field label="Badge Color"><Input value={toolModal.badgeColor||'#4a9b8e'} onChange={e=>setToolModal(t=>({...t,badgeColor:e.target.value}))}/></Field>
                    </div>
                  ...
                  <div className="flex justify-end gap-3 p-6 border-t border-white/10">
                    ...
                    <button disabled={savingTool} onClick={async()=>{
                      if(!toolModal.name.trim()){alert('Name is required');return;}
                      setSavingTool(true);
                      try{
                      ...
                      setSavingTool(false);
                    }} className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-white rounded-xl" style={{background:savingTool?'#2d6b62':'linear-gradient(135deg,#4a9b8e,#5bb8c4)'}}>
